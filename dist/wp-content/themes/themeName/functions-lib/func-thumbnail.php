...
function get_thumbnail_data($size = 'full') {
    if (has_post_thumbnail()) {
        ...
        $class         = '';
        // alt は未設定のケースがあるので、投稿タイトルをフォールバック
        $alt_text      = get_the_title(); // 投稿タイトルをaltに設定
        // $alt_text      = get_post_meta($thumbnail_id, '_wp_attachment_image_alt', true); // 画像に設定されているaltを使用する場合
        ...
        $width    = 106;   // ここを null にして属性を省略したいなら null に
        $height   = 88;
        $class    = 'u-no-image';
        $alt_text = 'no image';
    }
